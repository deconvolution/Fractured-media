...
%grid on;
%%
%save('C:\Users\Yi\OneDrive\master thesis\Schoenberg\m1_oil_pt','pt','-v7.3');
%save('C:\Users\Yi\OneDrive\master thesis\Schoenberg\m1_oil_p','p','-v7.3');
%% Model 1 water
phi(:)=.001;
rhof(:)=1000;
alpf(:)=1500;
...
rec2=pt(min(dims.mz),dims.mx,:);
rec=reshape(rec2,[size(rec2,2),size(rec2,3)])';
%save('C:\Users\Yi\OneDrive\master thesis\Schoenberg\m1_water_pt','pt','-v7.3');
%save('C:\Users\Yi\OneDrive\master thesis\Schoenberg\m1_water_p','p','-v7.3');
%% Model 1 gas
phi(:)=.001;
rhof(:)=600;
alpf(:)=400;
...
rec2=pt(min(dims.mz),dims.mx,:);
rec=reshape(rec2,[size(rec2,2),size(rec2,3)])';
%save('C:\Users\Yi\OneDrive\master thesis\Schoenberg\m1_gas_pt','pt','-v7.3');
%save('C:\Users\Yi\OneDrive\master thesis\Schoenberg\m1_gas_p','p','-v7.3');
%% Model 2 oil
phi(:)=.0001;
rhof(:)=600;
alpf(:)=1200;
...
set(gcf,'position',[80,80,1500,600]);
subplot(2,2,1)
imshow(real(rec_oil),.5*[min(real(rec_oil(:))),max(real(rec_oil(:)))]);
colorbar;
axis on;
%grid on;
ylim([0,300]);
...
subplot(2,2,2)
imshow(real(rec_water),.5*[min(real(rec_water(:))),max(real(rec_water(:)))]);
colorbar;
axis on;
%grid on;
ylim([0,300]);
...
subplot(2,2,3)
imshow(real(rec_water-rec_oil),1*[min(min(real(rec_water-rec_oil))),max(max(real(rec_water-rec_oil)))]);
colorbar;
axis on;
%grid on;
ylim([0,300]);
xlabel({['x*' num2str(dims.dh) '[m]']});
ylabel({['t*' num2str(dims.dt) '[s]']});
title({['rec difference']});

shg;

%pause(.1);
%print(gcf,['pt_water' num2str(i) '.png'],'-dpng','-r600');
%}